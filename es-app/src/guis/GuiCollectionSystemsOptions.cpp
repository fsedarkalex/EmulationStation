...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	if(CollectionSystemManager::get()->isEditing())
	{
		...
		message << boost::locale::format(_("FINISH EDITING '{1}' COLLECTION")) % Utils::String::toUpper(CollectionSystemManager::get()->getEditingCollection());
		row.addElement(std::make_shared<TextComponent>(mWindow, message.str(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
